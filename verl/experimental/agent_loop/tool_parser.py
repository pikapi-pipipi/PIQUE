...
class HermesToolParser(ToolParser):
    ...
    Only content after </think> is used for tool_call extraction: tool_calls and <question>
    inside <think>...</think> are ignored (they are "planning", not real actions).
    """
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        loop = get_event_loop()
        text = await loop.run_in_executor(None, self.tokenizer.decode, responses_ids)
        # Only use content after the last </think> for tool_call extraction (ignore <think>...</think>)
        if "</think>" in text:
            text_after_think = text.split("</think>")[-1].strip()
        else:
            text_after_think = text.strip()

        if self.tool_call_start_token not in text_after_think or self.tool_call_end_token not in text_after_think:
            return text_after_think, []

        matches = self.tool_call_regex.findall(text_after_think)
        function_calls = []
        for match in matches:
            try:
                function_call = json.loads(match)
        ...
        # remaining text exclude tool call tokens (still only from text after </think>)
        content = self.tool_call_regex.sub("", text_after_think)
