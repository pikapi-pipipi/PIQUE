...
def dtensor_to_full_tensor(param, device, non_blocking=True):
    """Convert DTensor to full tensor on device.

    Uses manual all_gather when the backend (e.g. NCCL) does not support
    allgather_into_tensor_coalesced, which can occur during update_weights.
    """
    try:
    ...
    if not isinstance(param, DTensor):
        return param.to(device, non_blocking=non_blocking)

    try:
        return param.to(device, non_blocking=non_blocking).full_tensor()
    except RuntimeError as e:
        if "allgather_into_tensor_coalesced" not in str(e):
            raise

    # Fallback: manual all_gather per mesh dimension (NCCL supports non-coalesced)
    local = param.to_local().to(device, non_blocking=non_blocking).contiguous()
    ...
    for mesh_dim in range(mesh.ndim):
        ...
        try:
            ...
            try:
                submesh = mesh[mesh_dim] if hasattr(mesh, "__getitem__") else mesh
                group = submesh.get_group()
            except Exception:
                group = mesh.get_group()
        world_size = dist.get_world_size(group=group)
        gather_list = [torch.empty_like(local) for _ in range(world_size)]
        dist.all_gather(gather_list, local, group=group)
        local = torch.cat(gather_list, dim=shard_dim).contiguous()
    return local
